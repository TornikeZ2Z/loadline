  ...
  return (
    ...
    >
      ...
      <g fill="none" strokeWidth="5.15" strokeLinecap="butt" strokeLinejoin="miter">
        <path d={RIBBON} stroke="var(--brand-navy, #17284A)" clipPath={`url(#${id}a)`} />
        <path d={RIBBON} stroke="var(--brand-blue, #1B6EF5)" clipPath={`url(#${id}b)`} />
        ...
        <path
          d="M 5.6 27.7 L 5.6 13.7"
          stroke="#fff"
          strokeWidth="1.5"
          strokeDasharray="2.9 2.8"
        />
      </g>
      {/* Lower left, nested in the M's own counter; upper right, standing over
          the blue plane. Diagonal to each other, which is the one thing a pair
          of pins on a load board is for. */}
      <Pin cx={12.2} cy={24} r={1.85} fill="var(--brand-blue, #1B6EF5)" />
      <Pin cx={21.6} cy={6.6} r={1.95} fill="var(--brand-green, #1FA463)" />
    </svg>
  );
}
...
export function Wordmark({
  ...
  return (
    <span className={className} style={{ fontWeight: 700, letterSpacing: "-0.02em", ...style }}>
      {/* "Mover" is not given a colour of its own: --text IS the brand navy
          now, so the first half of the wordmark is simply the page's ink. */}
      <span>Mover</span>
      <span style={{ color: "var(--accent)" }}>Mesh</span>
    </span>
  );
}
 ...
 */
export function Logo({ size = 28, fontSize, tagline = false, className }: LogoProps) {
  ...
  return (
    ...
    >
      ...
      <span style={{ display: "inline-flex", flexDirection: "column", gap: Math.max(2, Math.round(word * 0.2)) }}>
        <Wordmark className="leading-none" style={{ fontSize: word }} />
        {tagline ? (
          <span
            className="leading-none"
            style={{
              color: "var(--muted)",
              /* Never below the 12 px floor the mobile pass set for the app. */
